...
{
    // making new Customer
    currentTakeAwayCustomer = new takeAwayCustomer(age, name, quantity, coffeeName, bill);

    if (myCoffeeShop->nextTakeAwayCustomer == NULL)
    {
        // if first then insert in front
        myCoffeeShop->nextTakeAwayCustomer = currentTakeAwayCustomer;
    }
    else
    {
        // finding the last Node
        takeAwayCustomer *temp = myCoffeeShop->nextTakeAwayCustomer;
        ...
        {
            // insert at front
            takeAwayCustomer *firstCustomer = myCoffeeShop->nextTakeAwayCustomer;
            myCoffeeShop->nextTakeAwayCustomer = currentTakeAwayCustomer;
    ...
    cout << "Your Order has been Placed MR/MRS " << name << " and your order is " << coffeeName << " with " << quantity << " quantity and total bill is " << bill << endl;
}
void serveOrderTakeAwayCustomer()
{
    ...
    else
    {
        ...
        if(temp->next != NULL){
            myCoffeeShop->nextTakeAwayCustomer = temp->next;
        }
        else{
            myCoffeeShop->nextTakeAwayCustomer = NULL;
        }
...
void placeOrderDineInCustomer(int age, string name, string coffeeName, int quantity, double bill)
{
    ...
    if (myCoffeeShop->nextDineInCustomer == NULL)
    {
        // if first insert in front
        myCoffeeShop->nextDineInCustomer = currentDineInCustomer;
    }
    else
    {
        // finding the last Node
        dineInCustomer *temp = myCoffeeShop->nextDineInCustomer;
        while (temp->next != NULL)
        {
            temp = temp->next;
        }
...
void serveOrderDineInCustomer()
{
    ...
    else
    {
        ...
        dineInCustomer *temp = myCoffeeShop->nextDineInCustomer;
        if(temp->next != NULL){
            myCoffeeShop->nextDineInCustomer = temp->next;
        }
        else{
            myCoffeeShop->nextDineInCustomer = NULL;
        }
...
void placeOrderHomeDeliveryCustomer(int age, string name, string coffeeName, int quantity, double bill, string address, int deliveryCharges, int distanceDelivery)
{
    ...
    if (myCoffeeShop->nextHomeDeliveryCustomer == NULL)
    {
        // if first insert in front
        myCoffeeShop->nextHomeDeliveryCustomer = currentHomeDeliveryCustomer;
    }
    else
    {
        // finding the last Node
        homeDeliveryCustomer *temp = myCoffeeShop->nextHomeDeliveryCustomer;
        while (temp->next != NULL)
        {
            temp = temp->next;
        }
...
void serveOrderHomeDeliveryCustomer()
{
    ...
    else
    {
        ...
        homeDeliveryCustomer *first = myCoffeeShop->nextHomeDeliveryCustomer;

        if (first->next == NULL)
        {
            // if it is the only customer

            myCoffeeShop->nextHomeDeliveryCustomer = NULL;
...
void serveAllOrders()
{
    ...
    while (myCoffeeShop->nextTakeAwayCustomer != NULL)
    {
        serveOrderTakeAwayCustomer();
    }
    while (myCoffeeShop->nextDineInCustomer != NULL)
    {
        serveOrderDineInCustomer();
    }
    while (myCoffeeShop->nextHomeDeliveryCustomer != NULL)
    {
...
void displayAllOrdersTakeAwayCustomers()
{
    ...
    {
        takeAwayCustomer *traversal = myCoffeeShop->nextTakeAwayCustomer;
...
void displayAllOrdersHomeDeliveryCustomers()
{
    ...
    {
        homeDeliveryCustomer *traversal = myCoffeeShop->nextHomeDeliveryCustomer;
        while (traversal != NULL)
        {
            cout << "-----------------------------------------------------" << endl;
...
void displayAllOrdersDineInCustomers()
{
    ...
    {
        dineInCustomer *traversal = myCoffeeShop->nextDineInCustomer;
        while (traversal != NULL)
        {
            cout << "-----------------------------------------------------" << endl;
...
{
    takeAwayCustomer *p = myCoffeeShop->nextTakeAwayCustomer;
    while (p != NULL)
    {
        takeAway += p->cusotomer.bill;
        p = p->next;
    }
    dineInCustomer *q = myCoffeeShop->nextDineInCustomer;
    while (q != NULL)
    {
        dineIn += q->cusotomer.bill;
        q = q->next;
    }
    homeDeliveryCustomer *r = myCoffeeShop->nextHomeDeliveryCustomer;
    while (r != NULL)
    {
...
//                                0              1             2             3        4        5
string deliveryPoints[] = {"THEcoffeeSHOP", "Bahadurabad", "ShahFaisal", "G-Iqbal", "Johar", "DHA"};
...
vector<vector<pair<int, int>>> deliveryMap = {
    ...
    // first value in the pair is vertex and second is the distance (weight) in KM
    {{1, 2}, {2, 3}, {3, 5}, {5, 4}}, //  0  (The Coffee Shop)
    {{0, 2}, {5, 1}},                 //  1  (Bahadurabad)
    {{0, 3}, {3, 1}},                 //  2  (Shahfaisal)
    {{0, 5}, {4, 2}, {5, 2}, {2, 1}}, //  3  (Gulshan)
    {{3, 2}, {5, 2}},                 //  4  (Johar)
    {{0, 4}, {1, 1},{3,2} ,{4, 2}}    //  5  (DHA)
...
int main()
{
    ...
    // setting the name
    myCoffeeShop->shopName = "The Coffee Shop";

    // setting the address
    myCoffeeShop->address = "Tipu Sultan, Karachi";

    // Setting the menu
    myCoffeeShop->menu = new string[11]{"",
                                       "Cappucino", "Latte",
                                       "SpanishLatte", "CaramelLatte ",
                                       "MochaLatte", "FrenchVanilla",
                                       "Cortado", "Matchalatte",
                                       "PumpkinSpiceLatte", "HotChocolate"};

    // setting the price

    myCoffeeShop->price = new int[11]{0, 600, 550, 650, 700, 700, 800, 650, 800, 750, 500};
    ...
    do
    {
        ...
        cout << "-------------------------------------------------------------------------" << endl;
        cout << "---------------------------------" << myCoffeeShop->shopName << "-----------------------" << endl;
        cout << "-------------------------------------------------------------------------" << endl;
        cout << "-------------------------------------------------------------------------" << endl;

        cout << "Located at " << myCoffeeShop->address << endl;
        cout << "Our Menu is as follows: " << endl;
        for (int i = 1; i <= 10; i++)
        {
            cout << i << ". " << myCoffeeShop->menu[i] << " - " << myCoffeeShop->price[i] << endl;
        }
        ...
        { // placing order for take away customer
            ...
            cin >> quantity;
            cout << "Enter the option for the Coffee: ";
            cin >> coffeeIndex;

            bill = quantity * myCoffeeShop->price[coffeeIndex];
            placeOrderTakeAwayCustomer(age, name, myCoffeeShop->menu[coffeeIndex], quantity, bill);
        }
        break;
        ...
        {                                               // placing order for Home Delivery customer
            ...
            bill = quantity * myCoffeeShop->price[coffeeIndex] + deliveryCharges;

            // distance from the shop
            int distanceFromTheShop = distanceFromShop[optionDelivery];
            placeOrderHomeDeliveryCustomer(age, name, myCoffeeShop->menu[coffeeIndex], quantity, bill, address, distanceFromTheShop, deliveryCharges);
        }
        break;
        ...
        { // placing order for Dine-in customer
            ...
            bill = quantity * myCoffeeShop->price[coffeeIndex];
            placeOrderDineInCustomer(age, name, myCoffeeShop->menu[coffeeIndex], quantity, bill);
        }
        break;
    ...
    cout << "Coffee Order Management System -- Terminated" << endl;
